...
```{r}
pacman::p_load(sf, sfdep, sfExtras,
               tmap, leaflet,
               tidyverse,
...
Now that we have scoped out the dataset, we will now plot to see the BMR road network using **tmap**.
...
tm_shape(roads_bmr) +
  ...
  tm_layout(outer.bg.color = "#E4D5C9",
            bg.color = "#E4D5C9",
            frame = F,
            main.title = "Road Network in Bangkok Metropolitan Region",
...
accidents_bmr <- accidents_bmr %>% 
  ...
  select(-c("incident_datetime", "month" ,"day", "hour"))
```
...
4.  Finally, `select()` from dplyr is used to remove the *incident_datetime* column, as the necessary time-based variables have been derived from it.
